...
namespace imu_reader_sharp
{
    public partial class Form1 : Form
    {
        ...
        private void write_data(file file2write)
        {
            ...
            double[] corr_mag = { 2.4912, 2.4907, 2.4998, 2.5912, 3.4920, 2.5428, 2.4593, 2.5500, 2.5196, 2.5275, 1, 2.5017, 3.7391, 2.4499, 2.4844 };
            double[] corr_gyr = { 1.1111, 0.9333, 0.9333, 1.0185, 1.5122, 0.8355, 0.9148, 1.1154, 1, 1.1129, 1.1028, 1.1160, 1.5063, 0.9556, 0.9111 };
            double[,] zero_mag = { {0.1780, 0.1092, -0.0961}, {0.1259,  0.0621, -0.2346}, { 0.4120, 0.0386, 0.2578}, {0.2565, -0.0926, 0.0760}, { 0.1221, -0.0659, 0.2637},
                                   ...
                                   {0,      0,       0     }, {0.4010,  0.0459, -0.9670}, { 0.2514, 0.0564, 0.2268}, {0.0883, -0.0782, 0.0818}, { 0.2044,  0.0045, 0.2669} };
                    ...
                    (full_file[i] == 16) && (full_file[i + 1] == 49))   // условие начала IMU пакета
                {
                    ...
                    if (crc == pack[pack.Length - 1])
                    {
                        ...
                        if (type[k] == 49)
                        {
                            ...
                            m[k, 0] = -(double)BitConverter.ToInt16(buffer, 0) * 0.00030518 * corr_mag[block_index - 1] - zero_mag[block_index - 1, 0];
                            buffer[0] = pack[19]; buffer[1] = pack[20];
                            m[k, 1] = (double)BitConverter.ToInt16(buffer, 0) * 0.00030518 * corr_mag[block_index - 1] - zero_mag[block_index - 1, 1];
                            buffer[0] = pack[21]; buffer[1] = pack[22];
                            m[k, 2] = -(double)BitConverter.ToInt16(buffer, 0) * 0.00030518 * corr_mag[block_index - 1] - zero_mag[block_index - 1, 2];
        ...
        private double[] get_magn_coefs(int index)
        {
            ...
            switch (index)
            {
                ...
                case 13:
                    ...
                    break;
                case 14:
                    double[] temp14 = { 0.017646575899578,  0.034623366782344, -0.017481299993731, -0.015457509405639,
                                        0.012560817475161, -0.001658788388643,  0.015067669422511, -0.021144030526354,
                                       -0.011427533832203,  0.032545505486976, -0.096858721959977,  0.091484917001017 };
                    result = temp14;
                    break;
                case 15:
